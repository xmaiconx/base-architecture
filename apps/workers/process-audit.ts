...
import {
  ...
  type ProcessAuditPayload,
} from '@fnd/workers';

/**
 * Vercel Function wrapper for process-audit worker
...
export default async function handler(
  ...
  try {
    ...
    // Parse payload
    const payload: ProcessAuditPayload = req.body;

    // Create context and call handler
    const context = await createHandlerContext();
    await processAuditHandler(payload, context);

    res.status(200).json({ success: true });
  } catch (error) {
    console.error('Process audit handler error:', error);
    res.status(500).json({
