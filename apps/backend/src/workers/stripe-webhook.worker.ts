...
export class StripeWebhookWorker extends WorkerHost {
  ...
  async process(job: Job<StripeWebhookJobData>): Promise<void> {
    ...
    try {
      ...
      webhookEventId = await this.persistWebhookEvent(job.data);

      // 2. Process webhook based on event type
      await this.processWebhookEvent(job.data, webhookEventId);
   ...
   */
  private async persistWebhookEvent(data: StripeWebhookJobData): Promise<string> {
   ...
   */
  private async processWebhookEvent(data: StripeWebhookJobData, webhookEventId: string): Promise<void> {
    const { type, data: eventData } = data;

    switch (type) {
      case 'checkout.session.completed':
        await this.handleCheckoutCompleted(eventData.object, webhookEventId);
        break;

      case 'customer.subscription.created':
        await this.handleSubscriptionCreated(eventData.object, webhookEventId);
        break;

      case 'customer.subscription.updated':
        await this.handleSubscriptionUpdated(eventData.object, webhookEventId);
        break;

      case 'customer.subscription.deleted':
        await this.handleSubscriptionDeleted(eventData.object, webhookEventId);
        break;

      case 'invoice.payment_succeeded':
        await this.handlePaymentSucceeded(eventData.object, webhookEventId);
        break;

      case 'invoice.payment_failed':
        await this.handlePaymentFailed(eventData.object, webhookEventId);
        break;
   ...
   */
  private async handleCheckoutCompleted(session: any, webhookEventId: string): Promise<void> {
   ...
   */
  private async handleSubscriptionCreated(subscription: any, webhookEventId: string): Promise<void> {
   ...
   */
  private async handleSubscriptionUpdated(subscription: any, webhookEventId: string): Promise<void> {
   ...
   */
  private async handleSubscriptionDeleted(subscription: any, webhookEventId: string): Promise<void> {
   ...
   */
  private async handlePaymentSucceeded(invoice: any, webhookEventId: string): Promise<void> {
   ...
   */
  private async handlePaymentFailed(invoice: any, webhookEventId: string): Promise<void> {
